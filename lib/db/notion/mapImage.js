...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  if (!img && BLOG.RANDOM_IMAGE_URL && type === 'block') {
   return BLOG.RANDOM_IMAGE_URL.replace('{id}', block.id)
  }
  if (!img) {
   return null
}
  ...
  if (!isEmoji(ret)) {
    if (BLOG.RANDOM_IMAGE_URL && type === 'block') {
      // 只有配置了随机图片接口，才会替换图片
      ...
      if (isReplace) {
        ret = BLOG.RANDOM_IMAGE_URL.replace('{id}', block.id)
...
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  if (
    ...
    // picsum URL格式为 /seed/{id}/width/height，通过正则替换尺寸
    const picsumMatch = image.match(/picsum\.photos\/(seed\/[^/]+\/|id\/\d+\/)(\d+)\/(\d+)/)
    if (picsumMatch) {
      // 移动端使用更小尺寸
      const isMobile = typeof window !== 'undefined' && window.innerWidth < 768
      const targetWidth = isMobile ? 400 : width
      return image.replace(/\/\d+\/\d+\.jpg/, `/${targetWidth}/${Math.round(targetWidth * 0.75)}.jpg`)
    }
    return image
